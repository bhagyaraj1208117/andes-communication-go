...
import (
	...
	"github.com/bhagyaraj1208117/andes-communication-go/p2p/data"
	...
	atomicCore "github.com/bhagyaraj1208117/andes-core-go/core/atomic"
...
var (
	errExpectedor      = errors.New("expected error")
	providedTopic      = "topic"
			...
			topics := map[string]libp2p.PubSubTopic{
				providedTopic: &mock.PubSubTopicStub{
					PublishCalled: func(ctx context.Context, data []byte, opts ...pubsub.PubOpt) error {
						...
						return errExpectedor // return error for extra coverage only
					},
				},
			}
			args.Marshaller = &testscommon.MarshallerStub{
...
func TestMessagesHandler_RegisterMessageProcessor(t *testing.T) {
	...
	t.Run("known topic - add topic processors fails", func(t *testing.T) {
		...
		err := mh.RegisterMessageProcessor(providedTopic, providedIdentifier, &mock.MessageProcessorStub{})
		assert.True(t, errors.Is(err, errExpectedor))
	})
}
...
func TestMessagesHandler_Close(t *testing.T) {
	...
	defer checkForPanic(t)
	args := createMockArgMessagesHandler()
	errCloseCLB := fmt.Errorf("%w for CLB", errExpectedor)
	errCloseDebugger := fmt.Errorf("%w for debugger", errExpectedor)
	args.OutgoingCLB = &mock.ChannelLoadBalancerStub{
		CloseCalled: func() error {
			return errCloseCLB
		},
